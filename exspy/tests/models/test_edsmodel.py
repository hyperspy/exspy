...
class TestlineFit:
    ...
    def test_fit(self):
        ...
        np.testing.assert_allclose(
            [i.data for i in m.get_lines_intensity()], [[0.5], [0.2], [0.3]], atol=1e-4
        )
    ...
    def _check_model_store(self):
        ...
        assert [c.name for c in m] == [c.name for c in m1]
        assert [c.name for c in m.xray_lines] == [c.name for c in m1.xray_lines]
    ...
    def test_calibrate_xray_energy(self):
        ...
        np.testing.assert_allclose(
            ...
            elements.elements["Fe"]["Atomic_properties"]["Xray_lines"]["Ka"][
                "energy (keV)"
            ],
            atol=1e-6,
        )
    ...
    def test_calibrate_xray_weight(self):
        ...
        with pytest.warns(
            UserWarning, match="X-ray line expected to be in the model was not found"
        ):
            m.calibrate_xray_lines(
                calibrate="sub_weight", xray_lines=["Fe_Ka"], bound=100
            )
    ...
    def test_calibrate_xray_width(self):
        ...
        m.calibrate_xray_lines(calibrate="energy", xray_lines=["Fe_Ka"], bound=10)

        np.testing.assert_allclose(sigma, m["Fe_Ka"].sigma.value, atol=1e-2)
...
def test_comparison_quantification():
    ...
    quant_model = s.quantification(intensities_m, method="CL", factors=kfactors)
    ...
    atomic_percent = s2.quantification(intensities, method="CL", factors=kfactors)

    np.testing.assert_allclose(
        [q.data for q in quant_model], [q.data for q in atomic_percent], rtol=5e-2
    )
